...
class HardwareController:
    def __init__(self):
        ...
        self.ectool_available = False  # Is ectool detected and generally functional?
        self.ec_direct_available = False # Is actual EC Direct implemented and functional? (Will be False for now)
        self.hardware_ready = False # Is any control method active and ready?

        self.preferred_control_method: Optional[str] = "ectool" # Default preference, can be overridden by GUI
        self.active_control_method: str = "none" # 'ectool', 'ec_direct', or 'none'
        ...
        self.capabilities: Dict[str, bool] = {
            ...
            "ec_direct_access_ok": False, # True if EC Direct framework is present, user must implement logic
        }
    ...
    def set_control_method_preference(self, method: str):
        ...
        if method in ["ectool", "ec_direct"]:
            with self._lock: # Ensure thread safety when changing preference and potentially re-evaluating
    ...
    def _update_active_method_based_on_preference(self):
        # This method determines the active_control_method based on preferences and availability.
        # It's called after initial detection capabilities are known, or if preference changes.
        ...
        if preferred == "ec_direct":
            if self.ec_direct_available: # This would be True if user implements and _detect_ec_direct verifies
                new_active_method = "ec_direct"
                self.logger.info("EC Direct is available and preferred. Setting as active.")
            elif self.capabilities.get("ec_direct_access_ok"): # Framework is there, but not fully implemented
                new_active_method = "ec_direct" # Allow selection, methods will warn
            ...
            elif self.ec_direct_available: # Highly unlikely, but as a safe fallback
                new_active_method = "ec_direct"
                self.logger.warning("ectool preferred but not available. Oddly falling back to fully available EC Direct.")
            elif self.capabilities.get("ec_direct_access_ok"): # ectool preferred, not avail, but EC direct framework is
                new_active_method = "ec_direct"
        ...
        else: # No specific preference or unknown default
            if self.ectool_available:
                new_active_method = "ectool"
            elif self.ec_direct_available: # fully implemented EC Direct
                 new_active_method = "ec_direct"
            elif self.capabilities.get("ec_direct_access_ok"): # EC Direct framework only
                 new_active_method = "ec_direct"
        ...
        # Update overall hardware_ready status
        if self.active_control_method == "ectool":
            self.hardware_ready = self.ectool_available
        elif self.active_control_method == "ec_direct":
            # "Ready" means the framework is selected. Actual function depends on user implementation.
    ...
    def _perform_hardware_detection(self) -> None:
        ...
        time.sleep(0.1) # Small delay for preference to potentially be set by GUI calling set_control_method_preference
        ...
        try:
            self._detect_ectool()  # Determines self.ectool_available and ectool capabilities
            self._detect_ec_direct() # Determines self.ec_direct_available (will be False) and ec_direct_access_ok (will be True for framework)
            
            self._update_active_method_based_on_preference() # Sets self.active_control_method and self.hardware_ready
    ...
    def _detect_ectool(self) -> None:
        ...
        try:
            ...
            if not success_version:
                self.logger.warning(f"ectool 'version' command failed. Stderr: {stderr_version}, Stdout: {stdout_version}"); return # Critical fail

            # Test basic rgbkbd functionality
            ...
            # ectool is considered available if present, version OK, and critical functions (RGB or brightness) work.
            self.ectool_available = (self.capabilities["ectool_present"] and 
                                     self.capabilities["ectool_version_ok"] and
                                     (self.capabilities["ectool_rgbkbd_functional"] or self.capabilities["ectool_rgbkbd_clear_functional"]) and # Need at least one way to set colors
                                     (self.capabilities["ectool_pwmgetkblight_ok"] and self.capabilities["ectool_pwmsetkblight_ok"])) # Need brightness control
    ...
    def _detect_ec_direct(self) -> None:
        self.logger.info("Checking for EC Direct mode (placeholder detection)...")
        self.capabilities["ec_direct_access_ok"] = True # Signifies the software framework/pathway exists
        self.ec_direct_available = False # THIS MUST BE SET TO TRUE BY USER once implemented and verified

        if self.capabilities["ec_direct_access_ok"]:
            self.logger.warning("EC Direct mode framework is present in HardwareController.")
            if not self.ec_direct_available:
                self.logger.warning("EC DIRECT: LOW-LEVEL IMPLEMENTATION IS CURRENTLY MISSING.")
                self.logger.warning("To use EC Direct mode, you MUST:")
                self.logger.warning("  1. Implement platform-specific EC port I/O or kernel interface calls in the `_ec_direct_...` methods below (e.g., `_ec_direct_set_brightness`).")
                self.logger.warning("  2. Update THIS method (`_detect_ec_direct`) to truly verify EC access and functionality.")
                self.logger.warning("  3. If successfully implemented and verified, set 'self.ec_direct_available = True' within THIS method.")
                self.logger.warning("Without these changes, selecting 'EC Direct' will use placeholder functions that log warnings and likely fail to control hardware.")
        else:
            self.logger.error("EC Direct mode framework not marked as OK in capabilities (this is an internal logic error).")
    ...
    def _run_ectool_cmd_internal(self, args: List[str], timeout: float = 1.5, silent: bool = True) -> Tuple[bool, str, str]:
        ...
        if not self.capabilities.get("ectool_present", False): # Quick check
        ...
        except FileNotFoundError: # Should be caught by ectool_present, but as a safeguard
    ...
    # --- EC Direct Placeholder Methods ---
    def _ec_direct_set_brightness(self, brightness_percent: int) -> bool:
        self.logger.warning(f"EC DIRECT: Attempted to set brightness to {brightness_percent}%.")
        self.logger.warning("EC DIRECT: LOW-LEVEL IMPLEMENTATION for set_brightness IS MISSING.")
        self.logger.warning("EC DIRECT: Please implement hardware-specific EC commands in HardwareController._ec_direct_set_brightness().")
        # self.current_brightness = brightness_percent # Update cache if simulating success for testing framework
        return False # Return False to indicate not truly implemented for hardware

    def _ec_direct_get_brightness(self) -> Optional[int]:
        self.logger.warning("EC DIRECT: Attempted to get brightness.")
        self.logger.warning("EC DIRECT: LOW-LEVEL IMPLEMENTATION for get_brightness IS MISSING.")
        self.logger.warning("EC DIRECT: Please implement hardware-specific EC commands in HardwareController._ec_direct_get_brightness().")
        return None # Return None, or self.current_brightness if you want to return cached on non-impl.

    def _ec_direct_set_zone_color(self, zone_0_based: int, color: RGBColor) -> bool:
        self.logger.warning(f"EC DIRECT: Attempted to set zone {zone_0_based + 1} to {color.to_hex()}.")
        self.logger.warning("EC DIRECT: LOW-LEVEL IMPLEMENTATION for set_zone_color IS MISSING.")
        # self._zone_colors_cache[zone_0_based] = color # Update cache if simulating success
        return False

    def _ec_direct_set_all_leds_color(self, color: RGBColor) -> bool:
        self.logger.warning(f"EC DIRECT: Attempted to set all LEDs to {color.to_hex()}.")
        self.logger.warning("EC DIRECT: LOW-LEVEL IMPLEMENTATION for set_all_leds_color IS MISSING.")
        # self._zone_colors_cache = [color] * NUM_ZONES # Update cache if simulating success
        return False

    def _ec_direct_attempt_stop_hardware_effects(self) -> bool:
        self.logger.warning("EC DIRECT: Attempted to stop hardware effects.")
        self.logger.warning("EC DIRECT: LOW-LEVEL IMPLEMENTATION for stopping effects IS MISSING.")
        return False
    # --- End EC Direct Placeholder Methods ---
    ...
    def set_brightness(self, brightness_percent: int) -> bool:
        ...
        with self._lock:
            if self.active_control_method == "ectool":
                ...
                if result: self.current_brightness = brightness_percent # If it ever returns True
                return result
            else:
    ...
    def get_brightness(self) -> int:
        ...
        with self._lock:
            value_to_return = self.current_brightness # Default to cached
            ...
            if self.active_control_method == "ectool":
                ...
                # Warnings logged by _ec_direct_get_brightness itself
            else:
    ...
    def set_zone_colors(self, zone_colors: List[RGBColor]) -> bool:
        ...
        with self._lock:
            ...
            for i, color_obj in enumerate(zone_colors):
                ...
                if not self.set_zone_color(i + 1, color_obj): # Relies on the per-zone logic now
                    all_success = False
                # Delay only if using ectool and not the last command
    ...
    def set_all_leds_color(self, color: RGBColor) -> bool:
        ...
        with self._lock:
            if self.active_control_method == "ectool":
                ...
                # Fallback for ectool if 'clear' is not functional or failed
                return self.set_zone_colors([color] * NUM_ZONES) # Relies on per-zone logic
            elif self.active_control_method == "ec_direct":
            ...
            if self.active_control_method == "ectool":
                if self.capabilities.get("ectool_rgbkbd_demo_off_functional"):
                    ...
                    if success:
                        ...
                        return self.clear_all_leds() # Ensure a known state (black)
                    else:
                ...
                if result: # If EC direct stop was successful (conceptually)
                    return self._ec_direct_set_all_leds_color(RGBColor(0,0,0)) # Also clear via EC direct path
                return False # Placeholder failed
            else:
    ...
    def clear_all_leds(self) -> bool:
        ...
        return self.set_all_leds_color(RGBColor(0, 0, 0)) # This will use the active method's logic
    ...
    def get_hardware_info(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                "ectool_available_flag": self.ectool_available, # General ectool usability
                "ectool_executable": ECTOOL_EXECUTABLE,
                "ec_direct_available_flag": self.ec_direct_available, # Actual EC direct usability (false for now)
    ...
    def get_active_method_display(self) -> str:
        # Provides a GUI-friendly string for the active method
        if not self.detection_complete.is_set(): return "Detecting..."
        if self.active_control_method == "ectool": return "ectool"
        elif self.active_control_method == "ec_direct":
            if self.ec_direct_available: return "EC Direct (Implemented)" # Should not happen with current placeholders
    ...
    def is_operational(self) -> bool:
        if not self.detection_complete.is_set(): return False # Not operational if detection isn't even done
        return self.hardware_ready # Relies on hardware_ready being set correctly by _update_active_method
    ...
    def stop_current_effect(self):
        ...
        # Also try to stop any underlying hardware pattern if the active method supports it.
    ...
    def __del__(self):
        ...
        if not self._app_exiting_cleanly:
            ...
            try:
                # Ensure detection has run or timeout quickly
                if hasattr(self, 'detection_complete') and not self.detection_complete.is_set():
                    self.detection_complete.wait(timeout=0.2) # Short wait

                # Check active method before trying to clear
                if self.active_control_method == "ectool" and self.ectool_available:
                    ...
                    self.clear_all_leds() # Will call placeholder
                else:
