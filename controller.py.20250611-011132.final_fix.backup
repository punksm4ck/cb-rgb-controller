...
The application will continue without tray functionality."""
    ...
    def preview_static_color(self):
        ...
        # Immediately update the preview for static colors
        self.update_preview_keyboard()
        
        # Also update static preview canvas if it exists
    ...
    def start_current_effect(self):
        ...
        # Handle reactive effects specially - ADDED BY PATCH
...
The application will use fallback methods (ectool) until EC Direct is implemented."""
    ...
    def create_desktop_launcher(self):
        """Create desktop launcher in correct user location"""
        try:
            ...
            self.logger.info("Launcher details: Exec='" + exec_cmd + "', Path='" + str(working_dir_for_launcher) + "', FilenameBase='" + launcher_filename_base + "'")

        except Exception as e:
            self.logger.error("Could not determine execution paths for launcher: " + str(e), exc_info=True)
            if self.root.winfo_exists(): 
                messagebox.showerror("Launcher Error", "Could not determine script/package path: " + str(e), parent=self.root)
            return

        if platform.system() == "Linux":
            # Determine correct user and desktop location
            ...
            # Find the best desktop location
            desktop_dir = None
            ...
            if not desktop_dir:
                # Create applications directory as last resort
            ...
            # Create desktop file content using safe string concatenation
            content = (
                "[Desktop Entry]\n" +
                "Version=1.0\n" +
                "Name=" + APP_NAME + "\n" +
            ...
            try:
                ...
                msg = ("Desktop launcher created successfully:\n" +
                ...
                self.logger.info("Desktop launcher created: " + str(desktop_file_path))
            except Exception as e:
                log_error_with_context(self.logger, e, {"path": str(desktop_file_path)})
                if self.root.winfo_exists(): 
                    messagebox.showerror("Launcher Error", "Failed to write desktop file: " + str(e), parent=self.root)
        else:
            if self.root.winfo_exists(): 
    ...
    def preview_color_cycle(self, frame_count: int):
        ...
        speed_multiplier = self.get_hardware_synchronized_speed()
        # Reduced speed multiplier for color cycle to match hardware
        hue = (frame_count * speed_multiplier * 0.5) % 1.0  # Even slower for accurate sync
        rgb_float = colorsys.hsv_to_rgb(hue, 1.0, 1.0)
        color = RGBColor(int(rgb_float[0] * 255), int(rgb_float[1] * 255), int(rgb_float[2] * 255))
    ...
    def preview_pulse(self, frame_count: int):
        ...
        # Slower pulse to match hardware timing
    ...
    def preview_scanner(self, frame_count: int):
        ...
        # Much slower scanner to match hardware
    ...
    def preview_wave(self, frame_count: int):
        """FIXED: Hardware-synchronized wave effect - accurate zone timing"""
        try:
            base_color_rgb = RGBColor.from_hex(self.effect_color_var.get())
        except ValueError:
        ...
        # Much slower wave movement to match hardware exactly
    ...
    def preview_strobe(self, frame_count: int):
        """FIXED: Hardware-synchronized strobe effect"""
        try:
            base_color_rgb = RGBColor.from_hex(self.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(255,255,255)
        ...
        # Faster strobe to match hardware
        strobe_on = ((frame_count * speed_multiplier * 10) % 10) < 5
    ...
    def preview_ripple(self, frame_count: int):
        """FIXED: Hardware-synchronized ripple effect"""
        try:
            base_color_rgb = RGBColor.from_hex(self.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(0,255,255)
        ...
        center = NUM_ZONES // 2
        # Slower ripple to match hardware
        ripple_radius = (frame_count * speed_multiplier * 2) % (NUM_ZONES + 5)
    ...
    def preview_starlight(self, frame_count: int):
        """FIXED: Realistic starlight effect using individual key positions"""
        try:
            base_color_rgb = RGBColor.from_hex(self.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(255,255,255)
        ...
        if hasattr(self, 'key_grid') and self.key_grid:
            ...
            # Fallback to zone-based starlight
            for i in range(NUM_ZONES):
    ...
    def preview_raindrop(self, frame_count: int):
        """FIXED: Realistic raindrop effect using key grid positions"""
        ...
        if hasattr(self, 'key_grid') and self.key_grid:
            ...
            # Create raindrops falling from top to bottom
            num_drops = 3
            ...
            # Fallback to zone-based raindrop
            for i in range(NUM_ZONES):
                if is_rainbow:
    ...
    def intercept_hardware_signal_for_preview(self, effect_name, **params):
        """Intercept actual hardware signal for realistic preview"""
        if not hasattr(self, 'effect_manager') or not self.effect_manager:
            return False
        
        try:
            # Get the actual effect data that would be sent to hardware
            if hasattr(self.effect_manager, 'get_effect_preview_data'):
                preview_data = self.effect_manager.get_effect_preview_data(effect_name, **params)
                if preview_data:
                    self.apply_hardware_preview_data(preview_data)
                    return True
            
            # Fallback: simulate hardware timing
            return self.simulate_hardware_timing_for_preview(effect_name, **params)
        except Exception as e:
            self.logger.debug(f"Hardware signal interception failed: {e}")
            return False
    
    def apply_hardware_preview_data(self, preview_data):
        """Apply intercepted hardware data to preview"""
        try:
            if isinstance(preview_data, dict) and 'zones' in preview_data:
                for zone_idx, color_data in preview_data['zones'].items():
                    if 0 <= zone_idx < len(self.zone_colors):
                        if isinstance(color_data, dict):
                            self.zone_colors[zone_idx] = RGBColor.from_dict(color_data)
                        elif hasattr(color_data, 'r'):  # RGBColor object
                            self.zone_colors[zone_idx] = color_data
                
                self.update_preview_keyboard()
                return True
        except Exception as e:
            self.logger.debug(f"Failed to apply hardware preview data: {e}")
        
        return False
    
    def simulate_hardware_timing_for_preview(self, effect_name, **params):
        """Simulate realistic hardware timing for preview"""
        # This provides more accurate timing simulation
        timing_map = {
            'Color Cycle': {'base_speed': 0.05, 'speed_multiplier': 2.0},
            'Wave': {'base_speed': 0.08, 'speed_multiplier': 1.5},
            'Pulse': {'base_speed': 0.12, 'speed_multiplier': 3.0},
            'Scanner': {'base_speed': 0.15, 'speed_multiplier': 1.2},
            'Strobe': {'base_speed': 0.25, 'speed_multiplier': 4.0},
            'Ripple': {'base_speed': 0.10, 'speed_multiplier': 1.8},
            'Starlight': {'base_speed': 0.08, 'speed_multiplier': 1.0},
            'Raindrop': {'base_speed': 0.12, 'speed_multiplier': 1.5}
        }
        
        if effect_name in timing_map:
            timing_info = timing_map[effect_name]
            # Store timing info for use in preview methods
            self._current_effect_timing = timing_info
            return True
...
# Additional preview methods for comprehensive effect coverage
class AdvancedPreviewMethods:
    """Additional preview methods for extended effect support"""
    
    def __init__(self, gui_instance):
        self.gui = gui_instance
        
    def preview_wave_advanced(self, frame_count: int):
        """Advanced wave effect with multiple wave types"""
        try:
            base_color_rgb = RGBColor.from_hex(self.gui.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(0, 100, 255)
        
        is_rainbow = self.gui.effect_rainbow_mode_var.get()
        wave_type = (frame_count // 100) % 3  # Cycle through wave types
        
        for i in range(NUM_ZONES):
            if wave_type == 0:  # Sine wave
                wave_position = (frame_count * 0.3 + i * 2) % (NUM_ZONES * 4)
                intensity = (math.sin(wave_position * 0.5) + 1) / 2
            elif wave_type == 1:  # Triangle wave
                wave_position = (frame_count * 0.2 + i * 1.5) % (NUM_ZONES * 2)
                intensity = abs((wave_position % 2) - 1)
            else:  # Square wave
                wave_position = (frame_count * 0.4 + i * 3) % (NUM_ZONES * 2)
                intensity = 1.0 if (wave_position % 2) < 1 else 0.2
            
            if is_rainbow:
                hue = ((i + frame_count * 0.02) / NUM_ZONES) % 1.0
                rgb_float = colorsys.hsv_to_rgb(hue, 1.0, intensity)
                self.gui.zone_colors[i] = RGBColor(
                    int(rgb_float[0] * 255),
                    int(rgb_float[1] * 255),
                    int(rgb_float[2] * 255)
                )
        ...
        self.gui.update_preview_keyboard()

    def preview_spiral(self, frame_count: int):
        """Spiral effect rotating around zones"""
        try:
            base_color_rgb = RGBColor.from_hex(self.gui.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(255, 150, 0)
        
        is_rainbow = self.gui.effect_rainbow_mode_var.get()
        spiral_center = NUM_ZONES // 2
        spiral_radius = (frame_count * 0.1) % (NUM_ZONES + 2)
        spiral_angle = (frame_count * 0.2) % (2 * math.pi)
        
        for i in range(NUM_ZONES):
            distance_from_center = abs(i - spiral_center)
            angle_offset = (i - spiral_center) * 0.5
            spiral_position = spiral_radius * math.cos(spiral_angle + angle_offset)
            
            intensity = max(0, 1.0 - abs(distance_from_center - abs(spiral_position)) * 0.3)
            
            if is_rainbow:
                hue = (spiral_angle + angle_offset) % (2 * math.pi) / (2 * math.pi)
                rgb_float = colorsys.hsv_to_rgb(hue, 1.0, intensity)
                self.gui.zone_colors[i] = RGBColor(
                    int(rgb_float[0] * 255),
                    int(rgb_float[1] * 255),
    ...
    def preview_meteor(self, frame_count: int):
        ...
        # Multiple meteors with different speeds
        meteors = [
            {'pos': (frame_count * 0.5) % (NUM_ZONES + 5), 'tail_length': 3},
            {'pos': (frame_count * 0.3 + NUM_ZONES//2) % (NUM_ZONES + 5), 'tail_length': 4},
            {'pos': (frame_count * 0.7 + NUM_ZONES//4) % (NUM_ZONES + 5), 'tail_length': 2}
        ]
        
        # Initialize all zones to black
        for i in range(NUM_ZONES):
            self.gui.zone_colors[i] = RGBColor(0, 0, 0)
        
        # Draw meteors
        for meteor_idx, meteor in enumerate(meteors):
            meteor_pos = meteor['pos']
            tail_length = meteor['tail_length']
            
            for i in range(NUM_ZONES):
                if meteor_pos - tail_length <= i <= meteor_pos:
                    distance_from_head = meteor_pos - i
                    intensity = max(0, 1.0 - (distance_from_head / tail_length))
                    
                    if is_rainbow:
                        hue = (meteor_idx * 0.3 + frame_count * 0.01) % 1.0
                        rgb_float = colorsys.hsv_to_rgb(hue, 1.0, intensity)
                        color = RGBColor(
                            int(rgb_float[0] * 255),
                            int(rgb_float[1] * 255),
                    ...
                    existing = self.gui.zone_colors[i]
                    self.gui.zone_colors[i] = RGBColor(
                        min(255, existing.r + color.r),
                        min(255, existing.g + color.g),
                        min(255, existing.b + color.b)
                    )
        
        self.gui.update_preview_keyboard()

    def preview_fire(self, frame_count: int):
        """Fire simulation effect"""
        fire_colors = [
            RGBColor(255, 0, 0),    # Red
            RGBColor(255, 69, 0),   # Orange
        ...
        for i in range(NUM_ZONES):
            ...
            flicker_seed = (frame_count + i * 13) % 100
            base_intensity = 0.3 + 0.7 * (math.sin(flicker_seed * 0.2) + 1) / 2
            
            # Add randomness
            random_factor = (hash(f"{frame_count}-{i}") % 1000) / 1000.0
            intensity = base_intensity * (0.8 + 0.4 * random_factor)
            intensity = max(0, min(1, intensity))
            
            if is_rainbow:
                hue = (i / NUM_ZONES + frame_count * 0.02) % 1.0
                rgb_float = colorsys.hsv_to_rgb(hue, 1.0, intensity)
                self.gui.zone_colors[i] = RGBColor(
                    int(rgb_float[0] * 255),
                    int(rgb_float[1] * 255),
                ...
                # Choose fire color based on intensity
                color_index = int(intensity * (len(fire_colors) - 1))
    ...
    def preview_matrix(self, frame_count: int):
        """Matrix digital rain effect"""
        try:
            base_color_rgb = RGBColor.from_hex(self.gui.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(0, 255, 0)  # Matrix green
        ...
        for stream in range(num_streams):
            stream_pos = (frame_count * (0.4 + stream * 0.1) + stream * NUM_ZONES // num_streams) % (NUM_ZONES + 8)
            stream_length = 4
            
            for i in range(NUM_ZONES):
                if stream_pos - stream_length <= i <= stream_pos:
                    distance_from_head = stream_pos - i
                    intensity = max(0, 1.0 - (distance_from_head / stream_length) ** 2)
                    
                    # Add digital flicker
                    flicker = (hash(f"{frame_count//3}-{i}-{stream}") % 100) / 100.0
                    if flicker > 0.8:
                        intensity *= 1.5
                    ...
                    if is_rainbow:
                        hue = (stream * 0.2 + frame_count * 0.01) % 1.0
                        rgb_float = colorsys.hsv_to_rgb(hue, 1.0, intensity)
                        color = RGBColor(
                            int(rgb_float[0] * 255),
                            int(rgb_float[1] * 255),
    ...
    def preview_equalizer(self, frame_count: int):
        """Audio equalizer simulation effect"""
        try:
            base_color_rgb = RGBColor.from_hex(self.gui.effect_color_var.get())
        except ValueError:
            base_color_rgb = RGBColor(0, 255, 255)
        ...
        frequencies = []
        for i in range(NUM_ZONES):
            # Create pseudo-random frequency data
            freq_seed = (frame_count * 0.1 + i * 2.5) % 100
            amplitude = (math.sin(freq_seed * 0.3) + 1) / 2
            
            # Add some randomness for more realistic effect
            random_factor = (hash(f"{frame_count//2}-{i}") % 1000) / 1000.0
            amplitude = amplitude * (0.6 + 0.8 * random_factor)
            
            frequencies.append(amplitude)
        
        # Apply frequencies to zones
        for i in range(NUM_ZONES):
            intensity = frequencies[i]
            
            if is_rainbow:
                # Different hues for different frequency ranges
                hue = (i / NUM_ZONES) % 1.0
                rgb_float = colorsys.hsv_to_rgb(hue, 1.0, intensity)
                self.gui.zone_colors[i] = RGBColor(
                    int(rgb_float[0] * 255),
...
# Extended hardware compatibility and utility functions
class HardwareCompatibilityExtensions:
    """Extended hardware compatibility methods"""
    
    @staticmethod
    def detect_chromebook_model():
        """Detect specific Chromebook model for optimized support"""
        try:
            ...
            chromebook_models = {
                ...
                'ASUS': ['Chromebook', 'C302', 'C434', 'C536'],
                'Acer': ['Chromebook', 'CB3', 'CB5', 'R13'],
                'HP': ['Chromebook', 'x360', '14-da', '11-ae'],
                'Lenovo': ['Chromebook', 'C330', 'S330', '100e'],
                'Samsung': ['Chromebook', 'Plus', 'Pro', 'XE303'],
                'Dell': ['Chromebook', '3100', '3400', '5190']
            }
            
            for vendor_name, models in chromebook_models.items():
    ...
    @staticmethod
    def get_keyboard_backlight_paths():
    ...
    @staticmethod
    def test_keyboard_backlight_control():
        ...
        # Test sysfs paths
        paths = HardwareCompatibilityExtensions.get_keyboard_backlight_paths()
        ...
        ec_paths = [
            '/sys/kernel/debug/ec',
            '/dev/cros_ec',
            '/sys/bus/platform/devices/cros-ec-dev'
        ]
        
        results['chrome_ec_paths'] = []
        for path in ec_paths:
            if os.path.exists(path):
                results['chrome_ec_paths'].append(path)
        
        results['chrome_ec_available'] = len(results['chrome_ec_paths']) > 0
...
class AdvancedDiagnostics:
    ...
    def comprehensive_system_scan(self):
        ...
        scan_results['hardware_detection'] = {
            'chromebook_model': HardwareCompatibilityExtensions.detect_chromebook_model(),
            'backlight_paths': HardwareCompatibilityExtensions.get_keyboard_backlight_paths(),
            'backlight_test': HardwareCompatibilityExtensions.test_keyboard_backlight_control()
        }
        ...
        if scan_results['hardware_detection']['backlight_test']['ectool']:
            compatibility_score += 40
        else:
            issues.append("ectool not available")
            recommendations.append("Install ectool or cros-ec-utils package")
        
        if scan_results['hardware_detection']['backlight_test']['sysfs_available']:
            compatibility_score += 30
        else:
            issues.append("No sysfs backlight paths found")
        
        if scan_results['hardware_detection']['backlight_test']['chrome_ec_available']:
            compatibility_score += 20
        else:
            issues.append("Chrome EC not detected")
    ...
    def generate_support_report(self):
        ...
        report_lines = [
            "RGB KEYBOARD CONTROLLER - SYSTEM COMPATIBILITY REPORT",
            "=" * 60,
            f"Generated: {scan_results['timestamp']}",
            "",
            "SYSTEM INFORMATION:",
            f"Operating System: {scan_results['system_info']['os']} {scan_results['system_info']['os_release']}",
            f"Architecture: {scan_results['system_info']['architecture']}",
            f"Python Version: {scan_results['system_info']['python_version'].split()[0]}",
            f"Hostname: {scan_results['system_info']['hostname']}",
            "",
            "HARDWARE DETECTION:",
            f"Device Model: {scan_results['hardware_detection']['chromebook_model']}",
            f"Backlight Paths Found: {len(scan_results['hardware_detection']['backlight_paths'])}",
        ]
        
        for path in scan_results['hardware_detection']['backlight_paths']:
            report_lines.append(f"  - {path}")
        
        report_lines.extend([
            "",
            "CONTROL METHOD AVAILABILITY:",
            f"ectool: {'✓ Available' if scan_results['hardware_detection']['backlight_test']['ectool'] else '✗ Not Available'}",
            f"sysfs: {'✓ Available' if scan_results['hardware_detection']['backlight_test']['sysfs_available'] else '✗ Not Available'}",
            f"Chrome EC: {'✓ Available' if scan_results['hardware_detection']['backlight_test']['chrome_ec_available'] else '✗ Not Available'}",
            "",
            "SOFTWARE DEPENDENCIES:",
        ])
        
        for dep, available in scan_results['software_detection'].items():
        ...
        report_lines.extend([
            "",
            "COMPATIBILITY ASSESSMENT:",
            f"Overall Score: {scan_results['compatibility_assessment']['score']}/100",
            f"Rating: {scan_results['compatibility_assessment']['rating']}",
            "",
            "ISSUES DETECTED:",
        ])
        
        if scan_results['compatibility_assessment']['issues']:
            for issue in scan_results['compatibility_assessment']['issues']:
        ...
        if scan_results['compatibility_assessment']['recommendations']:
            for rec in scan_results['compatibility_assessment']['recommendations']:
                report_lines.append(f"  → {rec}")
        else:
            report_lines.append("  ✓ System appears to be properly configured")
        ...
        report_lines.extend([
            ...
            "3. Ensure proper permissions (run as root/administrator if needed)",
            "4. Test basic functionality with the hardware test tools",
            "",
            "For additional support, please share this report when seeking help.",
            "=" * 60
        ])
...
class EnhancedUtilities:
    """Enhanced utility functions for the RGB controller"""
    
    @staticmethod
    def validate_color_hex(hex_color):
        """Validate and normalize hex color strings"""
        if not hex_color:
            return False, "#000000"
        ...
        # Check length
        if len(hex_color) == 3:
            # Expand short form (e.g., 'fff' to 'ffffff')
        ...
        try:
            int(hex_color, 16)
            return True, f"#{hex_color.upper()}"
        except ValueError:
            return False, "#000000"
    
    @staticmethod
    def interpolate_colors(color1, color2, steps):
        """Interpolate between two colors with given number of steps"""
        if not isinstance(color1, RGBColor):
            color1 = RGBColor.from_hex(color1)
        if not isinstance(color2, RGBColor):
            color2 = RGBColor.from_hex(color2)
    ...
    @staticmethod
    def generate_rainbow_colors(count, saturation=1.0, value=1.0):
        """Generate rainbow colors with specified count"""
        colors = []
    ...
    def adjust_brightness(color, factor):
        ...
        return RGBColor(
            min(255, max(0, int(color.r * factor))),
            min(255, max(0, int(color.g * factor))),
            min(255, max(0, int(color.b * factor)))
        )
    
    @staticmethod
    def get_color_temperature(kelvin):
...
class PerformanceMonitor:
    ...
    def optimize_performance(self):
        ...
        if stats.get('average_frame_time', 0) > 0.05:  # > 50ms
            suggestions.append("Consider reducing animation frame rate")
        
        if stats.get('error_rate', 0) > 0.1:  # > 10% error rate
            suggestions.append("High error rate detected, check hardware connection")
        
        if stats.get('fps', 0) < 20:
            suggestions.append("Low FPS detected, consider simpler effects")
...
class SecurityValidator:
    """Security validation for RGB controller operations"""
    ...
    def validate_file_path(file_path, allowed_extensions=None):
        ...
        try:
            ...
            if allowed_extensions:
                if not any(str(resolved_path).lower().endswith(ext.lower()) for ext in allowed_extensions):
                    return False, f"File extension not in allowed list: {allowed_extensions}"
    ...
    def validate_settings_data(settings_data):
        """Validate settings data for security and integrity"""
        ...
        for key in settings_data.keys():
            if any(danger in str(key).lower() for danger in dangerous_keys):
                return False, f"Potentially dangerous key detected: {key}"
        ...
        expected_types = {
            ...
            'auto_apply_last_setting': bool,
            'minimize_to_tray': bool,
            'effect_rainbow_mode': bool
        }
        
        for key, expected_type in expected_types.items():
            if key in settings_data and not isinstance(settings_data[key], expected_type):
                return False, f"Invalid type for {key}: expected {expected_type.__name__}"
        
        # Validate value ranges
        if 'brightness' in settings_data:
            if not 0 <= settings_data['brightness'] <= 100:
                return False, "Brightness must be between 0 and 100"
        
        if 'effect_speed' in settings_data:
            if not 1 <= settings_data['effect_speed'] <= 10:
...
if hasattr(RGBControllerGUI, '__init__'):
    # Enhance the main GUI class with additional methods
    def enhanced_preview_methods_integration(self):
        """Integrate advanced preview methods"""
        if not hasattr(self, 'advanced_preview'):
            self.advanced_preview = AdvancedPreviewMethods(self)
            self.performance_monitor = PerformanceMonitor()
            self.diagnostics = AdvancedDiagnostics(self.logger)
    
    # Add the integration method to the class
    RGBControllerGUI.enhanced_preview_methods_integration = enhanced_preview_methods_integration
    
    # Add performance monitoring to preview updates
    original_update_preview = RGBControllerGUI.update_preview_keyboard
    
    def monitored_update_preview(self, canvas=None, elements_list=None):
        start_time = time.time()
        try:
            result = original_update_preview(self, canvas, elements_list)
    ...
    RGBControllerGUI.update_preview_keyboard = monitored_update_preview